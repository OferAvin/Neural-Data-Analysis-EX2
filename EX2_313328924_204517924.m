...
%% calculating VM fit for unit i    
    ...
    fitOpt = fitoptions (FitDeff_ornt);    			%determining start values for fit
    fitOpt.Lower       = [0   ,0  , -pi ];
    fitOpt.Upper       = [inf , inf	, pi  ];
    fitOpt.Startpoint  = [deg_max  , 5   , deg2rad(start_deg)];
    [fitResult_drct, GoF_drct] = fit(deg2rad(deg_vec)',...  %aplying fit for direction function 
        UnitsData.responseMean(unit_idx,:)', FitDeff_drct, fitOpt); 
    [fitResult_ornt, GoF_ornt] = fit(deg2rad(deg_vec)',...  %aplying fit for orientation function 
        UnitsData.responseMean(unit_idx,:)', FitDeff_ornt, fitOpt);
